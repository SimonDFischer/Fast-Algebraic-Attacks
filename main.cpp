...
/* new types */
typedef unsigned _int64	u64;	// 64-bit
...
int main() 
{
 ...
 /*-------------------------------------------------------*/
  ...
  //Compute N,E,D
  N = (u32)1<<n;
  for(E=0,i=0;i<=e;i++) {E+=(u32)bico(n,i);}
  for(D=0,i=0;i<=d;i++) {D+=(u32)bico(n,i);}
  
  //Prepare Array for Solutions CTf
  u8** SolutionLst = new u8*[E]; for(i=0; i<E; i++) {SolutionLst[i] = new u8[N];}

  u8* TTf = new u8[N]; u8* CTf = new u8[N]; 
  u8* TTg = new u8[N]; u8* CTg = new u8[N];
  u8* TTh = new u8[N]; u8* CTh = new u8[N];
  string ANF = ""; string table = ""; string text = "";
  ...
  {
	  if(c=='0') {if(i<N){TTf[i] = 0;} i++;}
	  if(c=='1') {if(i<N){TTf[i] = 1;} i++;}
  }
  if(i!=N) {error = 1; printf("\nError: Number of 0 and 1 in input file does not correspond to 2^n\n");}
  }
...
/*-------------------------------------------------------*/ 
  ...
  hadamard(TTf,CTf); degf = degree(CTf);

  //Display TT,CT,ANF of f
  table = "TT(f) = ["; ArrayToString(TTf,table); table += "]";
  //printf("Input f (deg f=%u)\n",degf);
  //printf("-------\n\n");
  //printf(table.c_str()); printf("\n");
  fprintf(fOutput,"Input f (deg f=%u)\n",degf);
  fprintf(fOutput,"-------\n\n");
  fprintf(fOutput,table.c_str()); fprintf(fOutput,"\n");

  table = "CT(f) = ["; ArrayToString(CTf,table); table += "]";
  //printf(table.c_str()); printf("\n");
  fprintf(fOutput,table.c_str()); fprintf(fOutput,"\n");
  ...
  fprintf(fOutput,ANF.c_str()); fprintf(fOutput,"\n\n");

  for(i=0; i<dim; i++) 
  {
	  //Choose a solution of CTg
	  for(j=0; j<N; j++) {CTg[j] = SolutionLst[i][j];}

	  //Compute all remaining CT and TT of g and h
	  hadamard(CTg,TTg); degg = degree(CTg);
	  for(j=0; j<N; j++) {TTh[j] = TTf[j] * TTg[j];}
	  hadamard(TTh,CTh); degh = degree(CTh);
	  ...
	  table = "TT(g) = ["; ArrayToString(TTg,table); table += "]";
      //printf(table.c_str()); printf("\n");
	  fprintf(fOutput,table.c_str()); fprintf(fOutput,"\n");

      table = "CT(g) = ["; ArrayToString(CTg,table); table += "]";
      //printf(table.c_str()); printf("\n"); 
	  fprintf(fOutput,table.c_str()); fprintf(fOutput,"\n");
	  ...
	  table = "TT(h) = ["; ArrayToString(TTh,table); table += "]";
      //printf(table.c_str()); printf("\n");
	  fprintf(fOutput,table.c_str()); fprintf(fOutput,"\n");

      table = "CT(h) = ["; ArrayToString(CTh,table); table += "]";
      //printf(table.c_str()); printf("\n");
	  fprintf(fOutput,table.c_str()); fprintf(fOutput,"\n");
  ...
  delete(SolutionLst);
  delete(TTf); delete(CTf); delete(TTg); delete(CTg); delete(TTh); delete(CTh); 
...
void ArrayToString(u8* A, string &str)
{
	u32 i;
	char buffer[33];
	for(i=0;i<N;i++) {str += itoa(A[i],buffer,10);}
}
...
{
	u32 weight = 0,i;
    for (i = 0; i < 32; i++,x>>=1) {weight += (u32)(x&1);}
    return weight;
}
...
u32 Algorithm1(u8* TT, u8** SolutionLst)
{
	...
	for(i=0;i<N;i++) {BetaLst[i]=0; GammaLst[i]=0;}

	for(j=0,i=0;i<N;i++)
	{if(hamming(i)>d) {GammaLst[j]=i; j++;}}

	for(j=0,i=0;i<N;i++)
	{if(hamming(i)<=e) {BetaLst[j]=i; j++;}}

	mat_GF2 M, Mt, Mk;
	M.SetDims( (long)rows, (long)cols);
	Mt.SetDims((long)cols, (long)rows);
	clear(M);  clear(Mt);  clear(Mk);
  ...
  for(i=0;i<E;i++) {for(j=0;j<N;j++) SolutionLst[i][j] = 0;}
  ...
  for(i=0; i<dim; i++) 
  {
	  ...
	  {
		  index = BetaLst[j];
		  if(Mk[i][j] == 1) {SolutionLst[i][index] = 1;} else {SolutionLst[i][index] = 0;}
	  }
  
  }//for dim

  delete(BetaLst); delete(GammaLst);
...
void dispEq(u8 *CT, string &ANF) 
{
	...
	for(i=0; i<N; i++) 
	{
		...
		{
			if(monomial & 1) 
			{ANF += "x"; ANF += itoa(j,buffer,10); ANF += " ";}//%d ",j);}
			monomial >>= 1;
		}
		flag = 1;
		}
...
float factln(int nn) {
  ...
  if (nn <= 1) return 0.0;
  if (nn <= 100) return a[nn] ? a[nn] : (a[nn]=gammln(nn+1.0)); // In range of table.
  else return gammln(nn+1.0); // Out of range of table.
}
float bico(int nn, int k)
